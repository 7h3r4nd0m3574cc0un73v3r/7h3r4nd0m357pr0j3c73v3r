...
import org.usayi.preta.scheduling.SchedulerStaticDAOImpl;
...
public class SchedulerServiceImpl implements SchedulerService
{
	...
	public void scheduleExpiring(Object entity)
	{
		...
		if( entity.getClass().equals( ShopSub.class)) {
			ShopSub shopSub = SchedulerStaticDAOImpl.getDao().loadShopSub( ((ShopSub) entity).getId());
			...
			scheduler.schedule( new ScheduledExpireTask( entity, notifications), new Date( shopSub.getEndDate().getTime() + margin));
		...
		if( entity.getClass().equals( AdvOffer.class)) {
			...
			/* Schedule Expiry */
			/* Give 30 seconds as delay to make sure the entity is really expired */
//			scheduler.schedule( new ScheduledExpireTask( entity, SchedulerStaticDAOImpl.getDao(), notifications), new Date( ad.getEndDate().getTime() + margin));
