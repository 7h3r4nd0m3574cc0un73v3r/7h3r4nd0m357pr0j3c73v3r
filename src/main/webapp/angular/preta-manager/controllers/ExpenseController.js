...
{	
	...
	function prepareEntity( entity) {
		...
		ExpenseService.loadArticleOrders( entity.id)
					  .then( function( r) {
	...
	$scope.$watch( 'entity', function( newValue) {
		$scope.setExpenseEAccount();
	}, true);
	
	$scope.setExpenseEAccount = function() {
		if( $scope.entity.adminEAccount != null && $scope.selection.count > 0) {
			var ids = [];
			...
			if( ids.length > 0) {
				ExpenseService.loadManagerEAccounts( ids)
				  .then( function( r) {
					  $scope.managerEAccounts = r.entities;
					  
					  angular.forEach( $scope.managerEAccounts, function( manEAccount) {
						 if( manEAccount.emp.id == $scope.entity.adminEAccount.emp.id) {
							 $scope.entity.eAccount = manEAccount;
							 console.info( $scope.entity.eAccount);
						 }
						 else {
							 $scope.entity.eAccount = null;
						 }
					  });
				  }, function( r) {
					  console.error( r);
