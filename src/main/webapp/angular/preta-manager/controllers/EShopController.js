...
App.controller( 'EShopController', [ '$state', '$stateParams', '$scope', '$rootScope',
                                     '$uibModal', 'EShopService', 'SubOfferService',
                                     'PaymentService',
                                    		   ...
                                    		   PaymentService)
{
	...
	PaymentService.loadAdminEAccountsForPayments()
				  .then( function( response) {
					  ...
					  console.log( $scope.adminEAccounts);
					  
					  $scope.$watch( 'form.entity.payment.eAccount', function( newValue, oldValue) {
						  	console.log( "Changed");
							if( $scope.form.entity.payment.eAccount != undefined) {
	...
	/* Loading EShops */
	EShopService.loadEntitiesByManager( $rootScope.loggedUser.userInfo.id, $scope.pagination.currentPage, $scope.pagination.pageSize)
						.then( function( response) {
							$scope.pagesNumber = response.pagesNumber;
							$scope.itemsNumber = response.itemsNumber;
	...
	/* Loading Entity for Details or Edit */
	if( $stateParams.eShopId != undefined) {
		EShopService.loadEntity( $stateParams.eShopId)
					.then( function( response) {
						$scope.entity = response;

						/* Show Pic Thumbnail on Update */
						if( $state.current.name == "root.e-shops.update")  {
							EShopService.fetchLogo( $scope.entity.id)
										.then( function( response) {
											$scope.mokujin.file = response;
										}, function( response) {
											console.error( response);
										});
						};
						...
						EShopService.loadArticlesCount( $scope.entity.id)
									.then( function( response) {
										$scope.entity.articlesCount = response;
									}, function( response) {
										console.error( response);
									});
						
						/* Passing variables */
						EShopService.activeEShop = $scope.entity;
						$scope.activeEShop = $scope.entity;
	...
	/* Loading SubOffers */
	/* TODO
	 * Otipm to load only on creation
	 */
	SubOfferService.loadEntities()
				   .then( function( response) {
					   $scope.subOffers = response.entities;
				   }, function( response) {
					   console.error( response);
				   });
